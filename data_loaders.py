...
def load_dataset(name, max_qubits=13, max_samples=1000, random_state=42):
    ...
    elif name == "cifar10":  # CIFAR-10 dataset
        try:
            # Try to import tensorflow first (common for CIFAR-10)
            import tensorflow as tf
            (X_train, y_train), (X_test, y_test) = tf.keras.datasets.cifar10.load_data()
            X = np.concatenate([X_train, X_test], axis=0)
            y = np.concatenate([y_train, y_test], axis=0).flatten()
            
            # Convert to grayscale and flatten to reduce dimensionality
            if len(X.shape) == 4:  # If RGB images (batch, height, width, channels)
                # Convert to grayscale using luminance formula
                X = np.dot(X[...,:3], [0.2989, 0.5870, 0.1140])
                X = X.reshape(X.shape[0], -1)  # Flatten
            elif len(X.shape) == 3:  # If already grayscale
                X = X.reshape(X.shape[0], -1)
                
        except ImportError:
            try:
                # Try pytorch as alternative
                import torch
                import torchvision
                ...
                # Last resort: use sklearn's fetch_openml
                from sklearn.datasets import fetch_openml
                print("Warning: Using OpenML version of CIFAR-10 which may be slower")
                X, y = fetch_openml('CIFAR_10', version=1, return_X_y=True, as_frame=False)
                y = y.astype(int)
                
                # CIFAR-10 from OpenML is already flattened (3072 features)
                # Convert to grayscale by averaging RGB channels
                if X.shape[1] == 3072:  # 32x32x3 flattened
                    X = X.reshape(-1, 3, 32*32)
                    X = np.mean(X, axis=1)  # Average RGB channels
